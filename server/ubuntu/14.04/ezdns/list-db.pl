...
my $opt = {};
if (!GetOptions($opt, qw/bdb|b:s output|o:s help|h/)) {
...
if (!defined $output_file || $output_file eq '') {
    usage('Please specify an output records file.');
    exit 1;
}
...
sub usage {
    ...
    print STDERR "usage: $0 --bdb=<bdb-file> --output=<output-file>\n\n";
    print STDERR "\tbdb-file: The output BerkeleyDB file you wish to create and use with bdbhpt-dynamic\n\n";
    print STDERR "\toutput-file: The output text-file containing records from BerkeleyDB file\n\n";
}
